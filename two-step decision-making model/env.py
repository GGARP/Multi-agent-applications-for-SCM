...
class DecentralizedInventoryEnvWithComm(gym.Env):
    """
    An inventory management environment with inter-agent communication.
    ...
    def __init__(
        ...
        holding_costs,
        stage_names,
        comm_size=4,  # updated dimension of communication
    ):
        super().__init__()
        ...
        #   - the communication messages from all agents (num_stages * comm_size)
    ...
    def update_state(self) -> None:
        """
        Update the environment state including current stage features, inventory, backlog, previous sales,
        arriving deliveries, and communication messages.
        """
        t = self.period
        ...
        # Incorporating communication messages from all agents.
        all_messages = self.messages.flatten()  # shape becomes (num_stages * comm_size,)
    ...
    def step(self, action_dict: dict[str, tuple]) -> tuple[dict, dict, dict, dict, dict]:
        ...
        :param action_dict: actions for each stage, where each action is a tuple:
        ...
        # Process actions to update orders and messages.
        ...
        for stage in range(self.num_stages):
            ...
            # Update the communication vector:
            # - Set element 1 to the clipped order quantity.
            # - Set element 3 to the current lead time for that stage.
            comm_vector[1] = float(clipped_order)
            comm_vector[3] = float(self.lead_times[stage])
        ...
        # Compute inventories at period end.
    ...
    def _parse_state(self, state: list) -> dict:
        ...
        return {
            ...
            'communications': communications  # full flattened communication vector
        }
...
def env_creator(env_config):
    ...
    if env_config is None:
        env_config = env_configs['constant_demand']
    ...
    return DecentralizedInventoryEnvWithComm(
        ...
        comm_size=4  # ensure this matches the updated communication vector size
    )
...
if __name__ == '__main__':
    ...
    print(f"parsed state_dict = {im_env.parse_state(im_env.state_dict)}")
    ...
    for t in range(im_env.num_periods):
        next_state_dict, rewards, terminations, truncations, infos = im_env.step(
            action_dict={f"stage_{m}": (4, np.zeros(im_env.comm_size, dtype=np.float32)) for m in range(im_env.num_stages)}
        )
        print('-' * 80)
        print(f"period = {t}")
        print(f"next_state_dict = {next_state_dict}")
        print(f"parsed next_state_dict = {im_env.parse_state(next_state_dict)}")
